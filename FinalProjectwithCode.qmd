...
SouthAmerica_only %>%
  ...
  theme_minimal() +
    theme(legend.title=element_text(size=9),
          ...
          plot.title.position = "plot")
```
Based on this survey data, we observe that most South America countries have young populations. Uruguay stands out as a country with a somewhat evenly distributed age distribution and with the most elderly people surveyed. Colombia also stands out because they have a large middle-aged population.  

```{r}
#seeing how much of our data set has considered emigrating for educational attainment purposes (very low at less than 1%)
summarise(SouthAmerica_only, mean(leave4educ))
...
countryleveldf <- as.data.frame(countryxeth_tbl)

countryleveldf

#replacing missing values with zero
countryleveldf <- within(countryleveldf, {
...
countryleveldf$total_pop <- rowSums(countryleveldf[, c('1', '2', '3', '4', '5', '7', 'NA')])
```


```{r}
#create a proportions ethnicity df out of the raw data 
eth_proportions_df <- countryleveldf %>%
...
eth_prop_df_long <-eth_proportions_df %>%
  select(-total_pop) %>%
  pivot_longer(!country_name, names_to = "score", values_to = "percent")

#creating a bar graph with the ethnicity proportions 
eth_prop_df_long %>%
  mutate(score = factor(score, labels = c("Category 1", "Category 2", "Category 3", "Category 4", "Category 5", "Category 7", "No Data"))) %>%
  ggplot(aes(x = country_name, y= percent, fill = score))+
  geom_bar(position="fill", stat="identity") + 
  labs(title = "Ethnicity Category as Proportion by Country", caption = "From Representative sample from LAPOP in 2023") +
  theme_minimal()

#use scales - categoralical varaible - fill manual and set colors with pallette 
#scale_fill_manuel (vector with categories, colors with hex codes)
...
gdp_geom <- sa_mapping %>%
  ...
  scale_fill_gradient(name = "gdpPercap", 
                      low = "#C0D8FD", high = "#0F6CFC",
